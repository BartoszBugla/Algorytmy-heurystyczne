...
class hba(IOptimizationAlgorithm):
    def __init__(self):
        ...
        self.params_info: List[ParamInfo] = [
            ParamInfo("population_base_size", "Population base size", 99999, 3),
            ParamInfo("t_max", "Number of generations", 99999, 1),
    ...
    def solve(
        ...
        candidates = np.random.uniform(
            low=lower_bound, high=upper_bound, size=(population_real_size, dimension)
        )
        ...
        for t in range(1, t_max + 1):
            ...
            for i in range(1, population_real_size):
                intensivity = np.square(candidates[i] - candidates[i - 1]) / (
                    4 * np.pi * (np.square(x_prey - candidates[i]) + eps)
                )
                ...
                if np.random.rand() < 0.5:
                    ...
                    x_new = (
                        x_prey
                        + (flag * param_b * intensivity * x_prey)
                        + flag
                        * np.random.rand()
                        * a
                        * (x_prey - candidates[i])
                        * np.abs(
                            ...
                            * (1 - np.cos(2 * np.pi * np.random.rand()))
                        )
                    )
                else:
                    flag = 1 if np.random.rand() <= 0.5 else -1
